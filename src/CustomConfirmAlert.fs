...
type Render<'Value> = ('Value->unit) -> ReactElement

type CustomConfirmAlert<'Value, 'Msg>(render: Render<'Value>, handler: ConfirmAlertResult -> 'Value option -> 'Msg) =
    let config = obj()
    ...
    interface ISweetAlert<'Msg> with
        member __.Run(dispatch) =
            async {
                let wrapper = Browser.Dom.document.createElement("div")
                ReactDom.render(ofFunction render Interop.setActionValue [], wrapper)
                Interop.setProp "content" wrapper.firstChild config

                let! result = Async.AwaitPromise (unbox (Interop.fire config))
                let value = Interop.getAs<obj> result "value"
                if isNull value then
                    let handle confirmResult = dispatch (handler confirmResult None)
                    ...
                    handler ConfirmAlertResult.Confirmed (Some (value :?> 'Value)) |> dispatch
            }
